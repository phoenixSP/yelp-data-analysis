...
for f in files:
    ...
    for chunk in reader:
        ...
        outfile = open(target_dataset_path + f+'_not_nulls.json', 'a')
        outfile.write(joined_df)
        review_df = pd.DataFrame([])
    print(f+"_not_nulls.json size without NULLs: "+str(os.path.getsize(target_dataset_path + f+'_not_nulls.json')))
